...
void Game::update() {
	if (spinning) {
		...
		if (rotationSpeed <= 0) {
			...
			double angle = Game::getWheelAngle(rotation, time);
			std::cout << "Angle " << rotation << "::" << time << std::endl;
...
double Game::getWheelAngle(double rotation, double time) {
	double angle = fmod(rotation * time, 360);
	return angle;
}
